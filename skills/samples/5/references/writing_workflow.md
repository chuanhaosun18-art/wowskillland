...
- 模板来源：默认使用 Skill 附带的北京交通大学硕士论文格式模板，也可使用用户指定模板。
...
1. 1 绪论：研究背景、意义、国内外研究现状、研究内容、创新点、论文结构。
2. 2 相关理论与研究现状：核心概念、理论基础、主流方法、关键技术、评价指标。
3. 3 研究方法与系统/模型设计：总体框架、关键模块、算法流程、公式推导、实现细节。
4. 4 实验设计与结果分析：数据、环境、指标、对��实验、消融实验、可视化、结果讨论。
5. 5 总结与展望：工作总结、不足、未来方向。
