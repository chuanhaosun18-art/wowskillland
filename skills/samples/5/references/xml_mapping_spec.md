...
- `toc_title`: 插图目录, 表格目录.
- `back_title`: 参考文献, 作者简历及攻读硕士学位期间取得的研究成果, 独创性声明, 学位论文数据集, 致谢, 作者简介, 附录.
...
- Display `[[EQ:...]]` formula numbers as `(chapter-index)` with an ASCII hyphen, for example `(3-1)`. The default `equationSeparator` is `"-"`. Override via `styleSettings.equationSeparator` if a different institution rule applies.
