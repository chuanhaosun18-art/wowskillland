...
- `[[FIG:description]]`: insert image placeholder paragraph plus caption. XML output uses `图X-Y`.
- `[[TBL:description]]`: insert table caption and a real Word `w:tbl` generated from structured rows or a known experiment-table description. XML output uses `表X-Y`; generated tables must be thesis three-line tables.
...
- Display formula numbering must use an ASCII hyphen between chapter and formula number, for example `(3-1)`, not `(3.1)` or `(3－1)`.
