...
| task_intent | **任务态（可执行）**：`thesis_topic` `resume_rewrite` `resume_jd_align` `report_structure` `mock_interview` `interview_review` `project_convergence` `literature_review` `content_script`；**编排态**：`zero_sum_competition` `resource_dependent` `direction_committed`（已决定方向后的编排）；**拒绝态（全拦）**：`emotional_support` `life_decision_undecided`；**编排输入（非独立 intent）**：`realtime_fact` |
...
编排项里带 `linked_skill_id` 的，点进去直接进 F4 工作台做那一件事。**这是三态之间唯一的正向通道**，也是产品完整性的体现：编排告诉你这周该改简历，任务态帮你把简历改完，改完之后固化成的 Skill 又能被下一份编排引用。

反向不通：任务态不会自动生成编排，因为编排必须有跨任务的 Path 作为来源。
